  ...
  document.getElementById("closeTicket").addEventListener("click", function(){
    ...
    try {
      ...
      fetch('http://127.0.0.1:5000/', options).then(response => response.json()).then((response) => {
          console.log('POST Requested');
          const reply = response;
          console.log(reply); // "Good Evening,\nI am glad we were able to help. We will be closing your ticket. If you have any questions, feel free to reach out to us by opening a new ticket or sending an email to helpdesk@gmail.com.\nRegards,\nEric Logan\nTech"
          // console.log('Copy to Clipboard');
          navigator.clipboard.writeText(reply).then(() =>{
            // clipboard successfully set
